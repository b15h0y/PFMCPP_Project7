...
void Character::defeatedAndLevelUp(int& initial, int& curr)
{
   if (curr < initial)
    {
        curr = initial;
...
void Character::attackInternal(Character& other)
{
    if( other.hitPoints <= 0 )
    {
      ...
      */
        defeatedAndLevelUp(*initialArmorLevel, armor);
        defeatedAndLevelUp(*initialHitPoints, hitPoints);
        defeatedAndLevelUp(*initialAttackDamage, attackDamage);
